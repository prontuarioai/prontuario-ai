  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)]">
      ...
      <div className="bg-white border-b border-gray-100 px-5 py-4">
        <h1 className="text-base font-bold text-gray-900">Chat da Equipe</h1>
            ...
            return (
              <div key={m.id} className={`flex ${isMeu ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] ${isMeu ? '' : 'flex items-end gap-2'}`}>
                  {!isMeu && (
                    <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-semibold text-xs shrink-0 mb-1">
                      {m.remetente_nome.charAt(0).toUpperCase()}
                    </div>
                  )}
                  <div className={`rounded-2xl px-3 py-2 ${
                    isMeu ? 'bg-brand-600 text-white rounded-br-sm' : 'bg-white text-gray-800 rounded-bl-sm shadow-sm'
                  }`}>
                    {!isMeu && (
                      <p className="text-xs font-semibold text-gray-500 mb-0.5">{m.remetente_nome}</p>
                    )}
      ...
      <div className="bg-white border-t border-gray-100 p-3">
        <form onSubmit={handleEnviar} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 disabled:bg-gray-50 disabled:text-gray-400"
