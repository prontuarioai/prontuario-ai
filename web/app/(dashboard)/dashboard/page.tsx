    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 space-y-2">
          ...
          {triagensRisco.map((t: any) => (
            <Link
              key={t.id}
              href={`/triagens/${t.id}`}
              className="flex items-center justify-between bg-white rounded-xl px-4 py-3 border border-red-100 hover:border-red-300 transition"
            >
              <span className="text-sm text-gray-900">{t.pacientes?.nome}</span>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-2">
          <h2 className="font-semibold text-gray-900">Evolução emocional</h2>
              ...
              <div className="space-y-2">
                {notificacoes.map((n: any) => (
                  <div key={n.id} className="flex items-start gap-2 py-1.5 border-b border-gray-50 last:border-0">
                    ...
                    <p className="text-xs text-gray-600 flex-1 leading-relaxed">{n.mensagem}</p>
                  </div>
                ))}
              </div>
            ) : (
