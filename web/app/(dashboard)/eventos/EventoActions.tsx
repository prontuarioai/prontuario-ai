    ...
    <div className="flex flex-col items-end gap-1 shrink-0">
      {!lido && (
        <button
          onClick={marcarLido}
          disabled={isPending}
          className="text-xs text-gray-400 hover:text-gray-600 dark:text-gray-300 transition-colors"
        >
          Marcar lido
        </button>
      )}
      ...
      {aberto && (
        <div className="absolute z-10 mt-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg p-4 w-64 space-y-3">
          <p className="text-xs font-semibold text-gray-700 dark:text-gray-200">Categoria</p>
          <div className="grid grid-cols-1 gap-1">
            {CATEGORIAS.map(c => (
              <button
                key={c.value}
                onClick={() => setCategoria(c.value)}
                className={`text-left text-sm px-3 py-1.5 rounded-lg transition-colors ${categoria === c.value ? 'bg-brand-50 text-brand-700 font-medium' : 'hover:bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-200'}`}
              >
                {c.label}
              </button>
            ))}
          ...
          <div>
            <p className="text-xs font-semibold text-gray-700 dark:text-gray-200 mb-1">Intensidade: {intensidade}/10</p>
          ...
          <div className="flex gap-2">
            <button onClick={() => setAberto(false)} className="flex-1 text-xs border border-gray-200 dark:border-gray-700 py-1.5 rounded-lg hover:bg-gray-50 dark:bg-gray-900">Cancelar</button>
